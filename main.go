...
func main() {
	...
	root := &cobra.Command{
		Use:           "radxa-penta-sata",
		Short:         "Fan control and OLED display for the Radxa Penta SATA HAT",
		SilenceUsage:  true,
		SilenceErrors: true,
	}
	root.PersistentFlags().StringVarP(&configPath, "config", "c", "radxa-penta-sata.conf", "path to config file")
	root.PersistentFlags().StringVar(&fontDir, "fonts", "/usr/share/consolefonts", "directory containing PSF fonts")
	root.PersistentFlags().StringVar(&socketPath, "socket", "radxa-penta-sata.sock", "path to control socket")
