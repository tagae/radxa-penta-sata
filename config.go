...
type Config struct {
	...
	Button  ButtonConfig  `ini:"button"`
	Slider  SliderConfig  `ini:"slider"`
	Display DisplayConfig `ini:"display"`
...
type ButtonConfig struct {
	ClickAction       string  `ini:"click-action"`
	DoubleClickAction string  `ini:"double-click-action"`
	PressAction       string  `ini:"press-action"`
	DoubleClickTime   float64 `ini:"double-click-time"`
	PressTime         float64 `ini:"press-time"`
}

type SliderConfig struct {
	Time              float64 `ini:"time"`
	TransitionOut     string  `ini:"transition-out"`
	TransitionIn      string  `ini:"transition-in"`
	TransitionSteps   int     `ini:"transition-steps"`
	TransitionFrameMs int     `ini:"transition-frame-ms"`
}
...
func LoadConfig(path string) *Config {
	cfg := &Config{
		...
		Button: ButtonConfig{ClickAction: "slider", DoubleClickAction: "none", PressAction: "none", DoubleClickTime: 0.7, PressTime: 1.8},
		Slider: SliderConfig{
			Time:              10,
			TransitionOut:     "slide-down",
			TransitionIn:      "slide-from-top",
			TransitionSteps:   8,
...
func (c *Config) GetFunc(key string) string {
	switch key {
	case "click":
		return c.Button.ClickAction
	case "twice":
		return c.Button.DoubleClickAction
	case "press":
		return c.Button.PressAction
	default:
		return "none"
	}
}
...
// GetDisks returns the current disk list.
