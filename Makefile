...
DEBFILES := debian/control debian/postinst debian/prerm debian/templates \
			debian/conffiles debian/radxa-penta-sata.service
...
$(DEB): $(BIN) $(ENVFILES) $(DEBFILES) radxa-penta-sata.conf
	rm -rf $(BUILDDIR)
	...
	# -- Hardware env files --
	install -d $(BUILDDIR)/usr/share/radxa-penta-sata/env
	install -m 644 env/*.env \
		$(BUILDDIR)/usr/share/radxa-penta-sata/env/

	# -- Configuration --
	install -d          $(BUILDDIR)/etc
	install -m 644 radxa-penta-sata.conf $(BUILDDIR)/etc/

	# -- Systemd service --
	install -d          $(BUILDDIR)/lib/systemd/system
	install -m 644 debian/radxa-penta-sata.service \
