...
func (f *Font) glyphIndex(ch rune) int {
	if f.unicodeMap != nil {
		...
		// PSF2 unicode table lookup failed; for Latin-1 runes fall back to
		// direct code-point indexing so characters like ° (U+00B0) still
		// render on fonts that store Latin-1 glyphs at their natural slots.
		if ch <= 0xFF {
			idx := int(ch)
			if idx < len(f.glyphs) {
				return idx
